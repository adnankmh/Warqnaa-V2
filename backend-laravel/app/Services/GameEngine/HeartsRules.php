...
class HeartsRules extends AbstractCardRules
{
    ...
    {
        if(($state['turn']??null)!==$playerId) return false; $card=(string)($payload['card']??'');
        if($action!=='play_card'||!in_array($card,$state['hands'][$playerId]??[],true)) return false;
        if(empty($state['trick']) && !$state['hearts_broken'] && $this->suit($card)==='hearts' && count(array_filter($state['hands'][$playerId],fn($c)=>$this->suit($c)!=='hearts'))>0) return false;
    ...
    {
        if(!$this->validate($state,$playerId,$action,$payload)){ $state['last_error']='invalid_hearts_action'; return $state; }
        $players=$state['players']; $card=(string)$payload['card']; $this->removeCard($state['hands'][$playerId],$card); if($this->suit($card)==='hearts')$state['hearts_broken']=true; $state['trick'][$playerId]=$card;
        if(count($state['trick'])>=count($players)){ $winner=$this->trickWinner($state['trick']); $pen=$this->penalty($state['trick']); $state['score'][$winner]=($state['score'][$winner]??0)+$pen; $state['round_penalties'][$winner]=($state['round_penalties'][$winner]??0)+$pen; $state['last_trick']=$state['trick']; $state['trick']=[]; $state['turn']=$winner; $state['messages'][]=$this->labelPlayer($winner).' أخذ عقوبة '.$pen.'.'; }
        else $state['turn']=$this->playerKeyNext($players,$playerId);
        if($this->allHandsEmpty($state['hands'])){ $state['phase']='finished'; $min=min($state['score']); $state['winner']=array_keys($state['score'],$min)[0]??null; $state['messages'][]='انتهت هارتس. الفائز الأقل عقوبة: '.$this->labelPlayer($state['winner']??''); }
        return $state;
    }
    private function penalty(array $trick): int { $p=0; foreach($trick as $c){ if($this->suit($c)==='hearts')$p-=1; if($c==='Q_spades')$p-=13; } return $p; }
}
