...
@if(auth()->user()?->hasAdminPermission('social_world'))
<section id="admin-social-world" class="admin-section r11-admin-world">
 <div class="r11-admin-hero"><div><span class="r11-kicker">R11 • BUILD 230 • CONTROL PLANE</span><h2>✦ Admin Social World</h2><p>مركز موحد للخصوصية والموجز والفعاليات والمدرجات والإعادات والهدايا، وكل إجراء إشرافي مسجل في Audit Log.</p></div><a class="btn" href="{{route('social-world')}}">معاينة الواجهة</a></div>
 <div class="r11-stats r11-admin-stats">@foreach(['activities'=>'منشورات','events'=>'فعاليات','spectators'=>'مشاهدون مباشرون','replays'=>'إعادات جاهزة','gifts'=>'هدايا'] as $key=>$label)<article><b>{{number_format($socialWorldStats[$key] ?? 0)}}</b><span>{{$label}}</span></article>@endforeach</div>
 <form class="pro-card r11-admin-settings" method="post" action="{{route('admin.social-world.settings')}}">@csrf
  <h3>إعدادات العالم الاجتماعي</h3><div class="r11-admin-switch-grid">
  @foreach(['social_world_enabled'=>'العالم الاجتماعي','social_feed_enabled'=>'الموجز','social_events_enabled'=>'الفعاليات','spectator_mode_enabled'=>'المدرجات','replay_system_enabled'=>'الإعادات','animated_gifts_enabled'=>'الهدايا المتحركة'] as $key=>$label)<label class="check-row"><input type="hidden" name="{{$key}}" value="0"><input type="checkbox" name="{{$key}}" value="1" {{$bool($key,true)?'checked':''}}> {{$label}}</label>@endforeach
  </div><div class="r11-admin-number-grid"><label>أقصى عدد مشاهدين<input type="number" name="max_room_spectators" min="1" max="10000" value="{{$val('max_room_spectators',120)}}"></label><label>حفظ الإعادات (يوم)<input type="number" name="replay_retention_days" min="1" max="365" value="{{$val('replay_retention_days',30)}}"></label><label>حجم الموجز<input type="number" name="social_feed_page_size" min="10" max="100" value="{{$val('social_feed_page_size',30)}}"></label></div><button class="primary">حفظ ونشر الإعدادات</button>
 </form>
 <div class="r11-admin-columns">
  <div class="pro-card"><h3>الموجز والإشراف</h3>@forelse($socialActivities as $item)<div class="r11-admin-row"><div><b>{{$item->actor?->profile?->display_name ?: $item->actor?->username}}</b><small>{{$item->type}} • {{$item->audience}} • {{$item->published_at}}</small><p>{{data_get($item->payload,'text','')}}</p></div><form method="post" action="{{route('admin.social-world.activity',$item)}}">@csrf<input type="hidden" name="action" value="{{$item->hidden?'restore':'hide'}}"><button class="{{$item->hidden?'':'danger'}}">{{$item->hidden?'استعادة':'إخفاء'}}</button></form></div>@empty<p class="muted">لا منشورات بعد.</p>@endforelse</div>
  <div class="pro-card"><h3>الفعاليات</h3>@forelse($socialEvents as $event)<div class="r11-admin-row"><div><b>{{$event->title['ar'] ?? 'فعالية'}}</b><small>{{$event->status}} • {{$event->starts_at}} • {{$event->creator?->username}}</small></div><form method="post" action="{{route('admin.social-world.event',$event)}}">@csrf<select name="action"><option value="{{$event->featured?'unfeature':'feature'}}">{{$event->featured?'إلغاء التثبيت':'تثبيت'}}</option><option value="live">مباشر</option><option value="finish">إنهاء</option><option value="cancel">إلغاء</option></select><button>تنفيذ</button></form></div>@empty<p class="muted">لا فعاليات بعد.</p>@endforelse</div>
  <div class="pro-card"><h3>الإعادات الآمنة</h3>@forelse($socialReplays as $replay)<div class="r11-admin-row"><div><b>{{$replay->room?->code}} • {{$replay->game?->key}}</b><small>{{$replay->status}} • {{$replay->frames_count}} frames • {{$replay->views}} views</small></div><form method="post" action="{{route('admin.social-world.replay',$replay)}}">@csrf<select name="action">@if($replay->status==='ready')<option value="{{$replay->featured?'unfeature':'feature'}}">{{$replay->featured?'إلغاء التثبيت':'تثبيت'}}</option><option value="hide">إخفاء</option>@elseif($replay->status==='hidden')<option value="restore">استعادة</option>@else<option value="hide">إيقاف وإخفاء</option>@endif</select><button>تنفيذ</button></form></div>@empty<p class="muted">لا إعادات بعد.</p>@endforelse</div>
  <div class="pro-card"><h3>المدرجات المباشرة</h3>@forelse($socialSpectators as $spectator)<div class="r11-admin-row"><div><b>{{$spectator->user?->profile?->display_name ?: $spectator->user?->username}}</b><small>{{$spectator->room?->code}} • {{$spectator->room?->game?->key}} • {{$spectator->last_seen_at?->diffForHumans()}}</small></div><form method="post" action="{{route('admin.social-world.spectator.evict',$spectator)}}" data-confirm="إخراج هذا المشاهد؟">@csrf<button class="danger">إخراج</button></form></div>@empty<p class="muted">لا مشاهدين نشطين.</p>@endforelse</div>
 </div>
</section>
@endif
