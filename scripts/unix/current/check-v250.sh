...
if command -v php >/dev/null 2>&1; then
  ...
  if [[ -f backend-laravel/vendor/autoload.php ]]; then
    (cd backend-laravel && php artisan test --filter V230SocialWorldTest && php artisan test --filter V240CompetitiveArenaTest && php artisan test --filter V250EngineGoldContractTest && php artisan warqna:competitive-tick --dry-run && php artisan test)
  else
