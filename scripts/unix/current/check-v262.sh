...
if command -v php >/dev/null 2>&1; then
  ...
  if [[ -f backend-laravel/vendor/autoload.php ]]; then
    (cd backend-laravel && php artisan test --filter V262AccountSecurityTest && php artisan test)
  fi
else
  echo "[INFO] PHP is absent; executable suites remain enforced by GitHub Actions."
fi
echo "WARQNAA R14.2 SECURE ACCOUNT BUILD 262 CHECK: PASS"
