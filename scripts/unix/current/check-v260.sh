...
if command -v php >/dev/null 2>&1; then
  ...
  if [[ -f backend-laravel/vendor/autoload.php ]]; then
    (cd backend-laravel && php artisan test --filter V250EngineGoldContractTest && php artisan test --filter V260GlobalReleaseContractTest && php artisan warqna:global-release-check --json && php artisan test)
  fi
else
  echo "[INFO] PHP is absent; executable suites remain enforced by GitHub Actions."
fi
echo "WARQNAA R14 GLOBAL RELEASE BUILD 260 CHECK: PASS"
