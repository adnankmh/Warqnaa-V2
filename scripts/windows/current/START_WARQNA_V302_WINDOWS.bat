...
cd /d "%~dp0\..\..\.."
title Warqnaa V1.1.2 Build 302
...
if not exist "backend-laravel\.env" (
  ...
  if not defined WARQNA_LOCAL_ADMIN_PASSWORD set "WARQNA_LOCAL_ADMIN_PASSWORD=LocalWarqnaa302ChangeMe"
...
start "Warqnaa Laravel B302" cmd /k "cd /d \"%CD%\backend-laravel\" && \"%WARQNA_PHP%\" artisan serve --host=127.0.0.1 --port=%WARQNA_PORT%"
...
if not errorlevel 1 (
  ...
  start "Warqnaa Flutter B302" cmd /k "cd /d \"%CD%\flutter_app\" && flutter pub get && flutter run -d chrome --dart-define=WARQNA_API_URL=http://127.0.0.1:%WARQNA_PORT%/api/mobile/v1 --dart-define=WARQNA_APP_VERSION=1.1.2 --dart-define=WARQNA_APP_BUILD=302"
) else (
