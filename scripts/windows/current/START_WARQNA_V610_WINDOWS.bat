...
popd
start "Warqnaa Laravel R6.1" cmd /k "cd /d \"%CD%\backend-laravel\" && \"%WARQNA_PHP%\" artisan serve --host=127.0.0.1 --port=%WARQNA_PORT%"
timeout /t 3 /nobreak >nul
start "" "http://127.0.0.1:%WARQNA_PORT%"
where flutter >nul 2>nul
if not errorlevel 1 start "Warqnaa Flutter R6.1" cmd /k "cd /d \"%CD%\flutter_app\" && flutter pub get && flutter run -d chrome --dart-define=WARQNA_API_URL=http://127.0.0.1:%WARQNA_PORT%/api/mobile/v1 --dart-define=WARQNA_APP_VERSION=1.4.0 --dart-define=WARQNA_APP_BUILD=610"
echo Warqnaa R6.1 running at http://127.0.0.1:%WARQNA_PORT%
exit /b 0
