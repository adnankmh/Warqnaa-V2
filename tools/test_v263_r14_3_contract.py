...
def main() -> None:
    ...
    check("Str::random(48)" in seeder and "LOCAL_ADMIN_PASSWORD_REQUIRED" not in text("flutter_app/lib/premium_v151.dart"), "administrator passwords are not shipped in app code")
    check("PRIVATE_LOCAL_ADMIN_EMAIL" not in text("backend-laravel/.env.example") and "LOCAL_ADMIN_PASSWORD_REQUIRED" not in text("backend-laravel/start-windows.bat"), "Windows and env templates do not publish administrator credentials")
