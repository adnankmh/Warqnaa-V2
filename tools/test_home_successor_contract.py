...
lobby = read('flutter_app/lib/r8_play_experience.dart')
...
assert r61_home_is_wired(main, home, lobby)
assert not r61_home_is_wired(main.replace('=> R61HomeDashboard(', '=> RemovedHome('), home, lobby)
assert not r61_home_is_wired(main, home, lobby.replace('onTab(1)', 'onTab(0)'))
assert not r61_home_is_wired(main, home.replace('=> R8HomeLobby(', '=> RemovedLobby('), lobby)
assert not r61_home_is_wired(main, home, lobby.replace('showGameLobby(context, controller, game)', 'showSettings(context, controller)'))
